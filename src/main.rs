...
impl ConfigOptions {
    fn read_toml(path: &Path) -> Result<ConfigOptions, ()> {
        ...
        Ok(ConfigOptions {
            verbose: false,
            syntax_theme: config.syntax_theme.unwrap_or_else(|| highlighter::DEFAULT_SYNTAX_THEME.to_owned()),
            theme: config.theme.unwrap_or_else(|| PathBuf::from("theme")),
            content_dir: config.content_dir.unwrap_or_else(|| PathBuf::from("content")),
    ...
    fn build_project(&self) {
        ...
        util::visit_dirs(self.content_dir.as_ref(), &|path| {
...
fn main() {
    let mut config = ConfigOptions::read_toml(Path::new("config.toml")).expect("Failed to open config.toml");
    ...
    info!("Took {} seconds", (time::precise_time_ns() - start_time) as f64 / (1_000_000_000 as f64));
}
